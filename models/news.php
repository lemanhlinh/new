...
//		}
		...
		function get_category_by_id($category_id)
		{
			if(!$category_id)
				return "";
			$query = " SELECT id,name,name_display,is_comment, alias, display_tags,display_title,display_sharing,
		...
		function getNews()
		{
			...
			$query = " SELECT id,title,content,category_id,category_alias, summary,hits, video, is_video,
                        alias, tags, created_time, updated_time,seo_title,image,optimal_seo,category_name,
	...
	/* 
        ...
        function get_products_related($products_related) {
    		...
    		$query = " SELECT id,name,summary,image, alias, category_id,category_alias,price_old,price,discount
    						  FROM " . $fs_table->getTable ( 'fs_products',1 ) . "
